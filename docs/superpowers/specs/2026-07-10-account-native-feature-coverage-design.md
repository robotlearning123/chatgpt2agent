...
- Also accept the live-account root-array envelope observed on 2026-07-10, whose items expose `id`, `name`, `marketplace_name`, `version`, and `enabled` without a nested release. This route returned 1,000 items while ignoring `limit=1`. For this variant, implement deterministic adapter-side pagination within one server process: derive a catalog fingerprint as a process-local keyed HMAC over the ordered, bounded, validated pre-redaction IDs, return at most `limit` items, and use an opaque local cursor containing only that HMAC and the next offset. A later page re-fetches the array and rejects a stale fingerprint as `contract_changed`; it never caches or embeds names, raw IDs, or account content in the cursor, and a cursor holder cannot test guessed raw IDs offline without the process-local key.
- Return only this allowlisted projection: `id`, `name`, `marketplace_name`, `display_name`, `version`, `enabled`, `scope`, `status`, `installation_policy`, `release_version`, `skill_names`, `disabled_skill_names`, `app_ids`, `app_template_ids`, `canonical_connector_ids`, `mcp_server_keys`, and string-valued `capability_names`. Every projected string is secret/PII-redacted; the boolean `enabled` is type-checked unchanged. Missing fields remain `null`; values are never fabricated across variants.
